...
describe('FormEngine — construction', () => {
...
// ════════════════════════════════════════════════════════════════════════════
// 2. Construction — schema validation
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — schema validation', () => {
    it('rejects non-object input', () => {
        expect(() => new FormEngine(null as any)).toThrow(FormDefinitionError)
        expect(() => new FormEngine('string' as any)).toThrow(FormDefinitionError)
    })

    it('rejects missing required top-level fields', () => {
        expect(() => new FormEngine({} as any)).toThrow(FormDefinitionError)
    })
    ...
    it('rejects additional top-level properties', () => {
        expect(
            () => new FormEngine({ ...baseDef([{ id: 1, type: 'string', label: 'A' }]), extra: true } as any),
        ).toThrow(FormDefinitionError)
    })

    it('rejects field with non-integer id', () => {
        expect(() => new FormEngine(baseDef([{ id: 1.5, type: 'string', label: 'A' }] as any))).toThrow(
            FormDefinitionError,
        )
    })

    it('rejects field with zero id', () => {
        expect(() => new FormEngine(baseDef([{ id: 0, type: 'string', label: 'A' }] as any))).toThrow(
            FormDefinitionError,
        )
    })

    it('rejects select without options', () => {
        expect(() => new FormEngine(baseDef([{ id: 1, type: 'select', label: 'S' }] as any))).toThrow(
            FormDefinitionError,
        )
    })

    it('rejects array without item', () => {
        expect(() => new FormEngine(baseDef([{ id: 1, type: 'array', label: 'A' }] as any))).toThrow(
            FormDefinitionError,
        )
    })

    it('rejects section with empty content', () => {
        expect(() => new FormEngine(baseDef([{ id: 1, type: 'section', title: 'S', content: [] }] as any))).toThrow(
            FormDefinitionError,
        )
    })

    it('rejects unknown field type', () => {
        expect(() => new FormEngine(baseDef([{ id: 1, type: 'unknown', label: 'A' }] as any))).toThrow(
            FormDefinitionError,
        )
    })
...
// ════════════════════════════════════════════════════════════════════════════
// 3. Construction — semantic errors
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — construction errors', () => {
    ...
    it('throws on string minLength > maxLength', () => {
        try {
            ...
            expect(e).toBeInstanceOf(FormDefinitionError)
            expect((e as FormDefinitionError).issues.some((i) => i.code === 'INVALID_RANGE_MIN_MAX_LENGTH')).toBe(true)
        }
    })
    ...
    it('throws on number min > max', () => {
        try {
            ...
            expect(e).toBeInstanceOf(FormDefinitionError)
            expect((e as FormDefinitionError).issues.some((i) => i.code === 'INVALID_RANGE_MIN_MAX')).toBe(true)
        }
    })
    ...
    it('throws on absolute date minDate > maxDate', () => {
        try {
            ...
            expect(e).toBeInstanceOf(FormDefinitionError)
            expect((e as FormDefinitionError).issues.some((i) => i.code === 'INVALID_RANGE_DATE')).toBe(true)
        }
    })

    it('does NOT throw for relative date range (cannot validate at definition time)', () => {
        expect(
            () =>
                new FormEngine(
                    baseDef([
                        {
                            ...
                            type: 'date',
                            label: 'D',
                            validation: { minDate: '+7d', maxDate: '+1d' },
                        },
                    ]),
                ),
        ).not.toThrow()
    ...
    it('throws on array minItems > maxItems', () => {
        try {
            ...
            expect(e).toBeInstanceOf(FormDefinitionError)
            expect((e as FormDefinitionError).issues.some((i) => i.code === 'INVALID_RANGE_ITEMS')).toBe(true)
        }
    })
...
describe('FormEngine — getFieldDef', () => {
    const engine = new FormEngine(
        baseDef([
            { id: 1, type: 'string', label: 'A' },
...
// ════════════════════════════════════════════════════════════════════════════
// 4. Visibility — isVisible
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — isVisible', () => {
    ...
    describe('simple condition operators', () => {
        const makeEngine = (op: string, value?: unknown) => {
            const condition: any = value !== undefined ? { field: 1, op, value } : { field: 1, op }
                ...
                new FormEngine(
                    baseDef([
                        { id: 1, type: 'number', label: 'N' },
                        {
                            ...
                            type: 'string',
                            label: 'D',
                            condition: { field: 1, op: op as any, value: 10 },
                        },
                    ]),
                )
...
// ════════════════════════════════════════════════════════════════════════════
// 5. Visibility — getVisibilityMap
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — getVisibilityMap', () => {
...
describe('FormEngine — getAffectedIds', () => {
...
// ════════════════════════════════════════════════════════════════════════════
// 7. Validation — string fields
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: string', () => {
    ...
    it('required string fails when missing', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'string', label: 'Name', validation: { required: true } }]),
        )
        expect(engine.validate(doc({})).valid).toBe(false)
        expect(engine.validate(doc({})).errors[0]!.rule).toBe('required')
        expect(engine.validate(doc({ '1': '' })).valid).toBe(false)
        expect(engine.validate(doc({ '1': null })).valid).toBe(false)
    })

    it('optional string passes when missing', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'string', label: 'Name', validation: { minLength: 2 } }]))
        expect(engine.validate(doc({})).valid).toBe(true)
        expect(engine.validate(doc({ '1': '' })).valid).toBe(true)
    })

    it('minLength error', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'string', label: 'S', validation: { minLength: 5 } }]))
        const result = engine.validate(doc({ '1': 'ab' }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('minLength')
        expect(result.errors[0]!.params).toEqual({ minLength: 5, actual: 2 })
    })

    it('maxLength error', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'string', label: 'S', validation: { maxLength: 3 } }]))
        const result = engine.validate(doc({ '1': 'toolong' }))
        expect(result.valid).toBe(false)
    ...
    it('pattern validation passes', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'string', label: 'S', validation: { pattern: '^\\d{3}$' } }]),
        )
        expect(engine.validate(doc({ '1': '123' })).valid).toBe(true)
    })

    it('pattern validation fails with default message', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'string', label: 'S', validation: { pattern: '^\\d{3}$' } }]),
        )
        const result = engine.validate(doc({ '1': 'abc' }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('pattern')
        expect(result.errors[0]!.message).toContain('pattern')
    })

    it('pattern validation fails with custom patternMessage', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
                    type: 'string',
                    label: 'S',
                    validation: { pattern: '^\\d{3}$', patternMessage: 'Must be 3 digits' },
                },
            ]),
        )
        const result = engine.validate(doc({ '1': 'abc' }))
        expect(result.errors[0]!.message).toBe('Must be 3 digits')
    })

    it('type error when non-string provided for string field', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'string', label: 'S' }]))
        const result = engine.validate(doc({ '1': 42 }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('type')
    })

    it('minLength and maxLength can trigger simultaneously', () => {
        // A value that is too short for minLength and somehow also triggers maxLength — impossible in practice.
        // Instead test that both are checked independently:
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'string', label: 'S', validation: { minLength: 3, maxLength: 5 } }]),
        )
        expect(engine.validate(doc({ '1': 'ab' })).errors[0]!.rule).toBe('minLength')
        expect(engine.validate(doc({ '1': 'toolong' })).errors[0]!.rule).toBe('maxLength')
        expect(engine.validate(doc({ '1': 'abc' })).valid).toBe(true)
    })
})

// ════════════════════════════════════════════════════════════════════════════
// 8. Validation — number fields
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: number', () => {
    it('valid number passes', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'number', label: 'N', validation: { required: true, min: 0, max: 100 } }]),
        )
        expect(engine.validate(doc({ '1': 50 })).valid).toBe(true)
    })

    it('required number fails when missing', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N', validation: { required: true } }]))
        expect(engine.validate(doc({})).valid).toBe(false)
        expect(engine.validate(doc({})).errors[0]!.rule).toBe('required')
    })

    it('optional number passes when missing', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N', validation: { min: 0 } }]))
        expect(engine.validate(doc({})).valid).toBe(true)
    })

    it('min error', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N', validation: { min: 10 } }]))
        const result = engine.validate(doc({ '1': 5 }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('min')
    })

    it('max error', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N', validation: { max: 10 } }]))
        const result = engine.validate(doc({ '1': 15 }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('max')
    })

    it('boundary values: min=10, value=10 passes', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N', validation: { min: 10 } }]))
        expect(engine.validate(doc({ '1': 10 })).valid).toBe(true)
    })

    it('boundary values: max=10, value=10 passes', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N', validation: { max: 10 } }]))
        expect(engine.validate(doc({ '1': 10 })).valid).toBe(true)
    })

    it('type error when string provided for number field', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N' }]))
        const result = engine.validate(doc({ '1': 'not-a-number' }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('type')
    })

    it('zero is a valid value (not treated as empty)', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'number', label: 'N', validation: { required: true } }]))
        expect(engine.validate(doc({ '1': 0 })).valid).toBe(true)
    })

    it('negative numbers work', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'number', label: 'N', validation: { min: -100, max: -1 } }]),
        )
        expect(engine.validate(doc({ '1': -50 })).valid).toBe(true)
        expect(engine.validate(doc({ '1': 0 })).valid).toBe(false)
    })
})

// ════════════════════════════════════════════════════════════════════════════
// 9. Validation — boolean fields
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: boolean', () => {
    it('required boolean passes with true', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'boolean', label: 'B', validation: { required: true } }]))
        expect(engine.validate(doc({ '1': true })).valid).toBe(true)
    })

    it('required boolean passes with false', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'boolean', label: 'B', validation: { required: true } }]))
        expect(engine.validate(doc({ '1': false })).valid).toBe(true)
    })

    it('required boolean fails when undefined', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'boolean', label: 'B', validation: { required: true } }]))
        expect(engine.validate(doc({})).valid).toBe(false)
        expect(engine.validate(doc({})).errors[0]!.rule).toBe('required')
    })

    it('required boolean fails when null', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'boolean', label: 'B', validation: { required: true } }]))
        expect(engine.validate(doc({ '1': null })).valid).toBe(false)
    })

    it('optional boolean passes when missing', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'boolean', label: 'B' }]))
        expect(engine.validate(doc({})).valid).toBe(true)
    })
})

// ════════════════════════════════════════════════════════════════════════════
// 10. Validation — date fields
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: date', () => {
    it('valid date passes', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'date', label: 'D', validation: { required: true } }]))
        expect(engine.validate(doc({ '1': '2025-06-15' })).valid).toBe(true)
    })

    it('required date fails when missing', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'date', label: 'D', validation: { required: true } }]))
        expect(engine.validate(doc({})).valid).toBe(false)
    })

    it('invalid date string fails', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'date', label: 'D' }]))
        const result = engine.validate(doc({ '1': 'not-a-date' }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('invalidDate')
    })

    it('type error for non-string value in date field', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'date', label: 'D' }]))
        const result = engine.validate(doc({ '1': 12345 }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('type')
    })

    it('absolute minDate/maxDate pass for value in range', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
                    type: 'date',
                    label: 'D',
                    validation: { minDate: '2025-01-01T00:00:00.000Z', maxDate: '2025-12-31T00:00:00.000Z' },
                },
            ]),
        )
        expect(engine.validate(doc({ '1': '2025-06-15' })).valid).toBe(true)
    ...
    it('absolute minDate fails for value before range', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'date', label: 'D', validation: { minDate: '2025-06-01T00:00:00.000Z' } }]),
        )
        const result = engine.validate(doc({ '1': '2025-05-01' }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('minDate')
    })

    it('absolute maxDate fails for value after range', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'date', label: 'D', validation: { maxDate: '2025-06-01T00:00:00.000Z' } }]),
        )
        const result = engine.validate(doc({ '1': '2025-07-01' }))
        expect(result.valid).toBe(false)
    ...
    it('optional date passes when empty', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'date', label: 'D', validation: { minDate: '2025-01-01T00:00:00.000Z' } }]),
        )
        expect(engine.validate(doc({})).valid).toBe(true)
        expect(engine.validate(doc({ '1': '' })).valid).toBe(true)
    })
...
// ════════════════════════════════════════════════════════════════════════════
// 11. Validation — select fields
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: select', () => {
    const options = [
        { value: 'r', label: 'Red' },
        { value: 'g', label: 'Green' },
        { value: 'b', label: 'Blue' },
    ...
    it('valid option passes', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'select', label: 'C', options, validation: { required: true } }]),
        )
        expect(engine.validate(doc({ '1': 'r' })).valid).toBe(true)
    })

    it('required select fails when missing', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'select', label: 'C', options, validation: { required: true } }]),
        )
        expect(engine.validate(doc({})).valid).toBe(false)
    ...
    it('invalid option fails', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'select', label: 'C', options }]))
        const result = engine.validate(doc({ '1': 'purple' }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('invalidOption')
    })

    it('numeric option value works', () => {
        const numOptions = [
            { value: 1, label: 'One' },
            { value: 2, label: 'Two' },
        ]
        const engine = new FormEngine(baseDef([{ id: 1, type: 'select', label: 'S', options: numOptions }]))
        expect(engine.validate(doc({ '1': 1 })).valid).toBe(true)
        expect(engine.validate(doc({ '1': 3 })).valid).toBe(false)
    })

    it('optional select passes when missing', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'select', label: 'C', options }]))
        expect(engine.validate(doc({})).valid).toBe(true)
    })
})
...
// ════════════════════════════════════════════════════════════════════════════
// 12. Validation — array fields
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: array', () => {
    it('valid array passes', () => {
        ...
        expect(engine.validate(doc({ '1': ['a', 'b'] })).valid).toBe(true)
    })
    ...
    it('minItems error', () => {
        ...
        const result = engine.validate(doc({ '1': ['a'] }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('minItems')
    })
    ...
    it('maxItems error', () => {
        ...
        const result = engine.validate(doc({ '1': ['a', 'b', 'c'] }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('maxItems')
    })

    it('empty array passes when no minItems', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
    ...
    it('null/undefined array passes (treated as empty)', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
        ...
        expect(engine.validate(doc({})).valid).toBe(true)
        expect(engine.validate(doc({ '1': null })).valid).toBe(true)
    })

    it('non-array value fails with type error', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
        ...
        const result = engine.validate(doc({ '1': 'not-an-array' }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.rule).toBe('type')
    })

    it('validates individual items — string items with minLength', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
        ...
        const result = engine.validate(doc({ '1': ['abc', 'ab', 'abcd'] }))
        expect(result.valid).toBe(false)
    ...
    it('validates individual items — number items with min/max', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
        ...
        const result = engine.validate(doc({ '1': [50, -1, 101] }))
        expect(result.valid).toBe(false)
    ...
    it('validates array of select items', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
        ...
        expect(engine.validate(doc({ '1': ['r', 'g'] })).valid).toBe(true)
        const result = engine.validate(doc({ '1': ['r', 'purple'] }))
        expect(result.valid).toBe(false)
        expect(result.errors[0]!.itemIndex).toBe(1)
        expect(result.errors[0]!.rule).toBe('invalidOption')
    })
    ...
    it('array level errors + item level errors combined', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
        ...
        // 3 items (exceeds maxItems=2), and the second item is too short
        const result = engine.validate(doc({ '1': ['abc', 'ab', 'def'] }))
        expect(result.valid).toBe(false)
        const rules = result.errors.map((e) => e.rule)
        expect(rules).toContain('maxItems')
...
describe('FormEngine — validate: file', () => {
    const validFile = { name: 'doc.pdf', mimeType: 'application/pdf', size: 1024, url: 'https://example.com/doc.pdf' }

    it('constructs with a file field', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'file', label: 'Resume' }]))
        expect(engine.getFieldDef(1)?.type).toBe('file')
    })

    it('required file passes with valid value', () => {
        const engine = new FormEngine(
    ...
    it('required file fails when missing', () => {
        const engine = new FormEngine(
            baseDef([{ id: 1, type: 'file', label: 'Resume', validation: { required: true } }]),
        )
    ...
    it('set/notset conditions work with file field', () => {
        const engine = new FormEngine(
            baseDef([
                { id: 1, type: 'file', label: 'Upload' },
                { id: 2, type: 'string', label: 'Details', condition: { field: 1, op: 'set' } },
            ]),
        )
        expect(engine.isVisible(2, doc({ '1': validFile }))).toBe(true)
        expect(engine.isVisible(2, doc({}))).toBe(false)
    })

    it('hidden required file field produces no error', () => {
        const engine = new FormEngine(
            baseDef([
                { id: 1, type: 'boolean', label: 'HasFile' },
                {
                    id: 2,
                    type: 'file',
                    label: 'Resume',
    ...
    it('validates array of file items', () => {
        const engine = new FormEngine(
            baseDef([
                {
                    id: 1,
...
// ════════════════════════════════════════════════════════════════════════════
// 13. Validation — hidden fields are skipped
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: hidden fields skipped', () => {
...
// ════════════════════════════════════════════════════════════════════════════
// 14. Validation — sections are never validated
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: sections not validated', () => {
...
// ════════════════════════════════════════════════════════════════════════════
// 15. Validation — no validation rules
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: fields without validation rules', () => {
...
// ════════════════════════════════════════════════════════════════════════════
// 16. Validation — multiple fields and error aggregation
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — validate: multiple fields', () => {
...
// ════════════════════════════════════════════════════════════════════════════
// 18. Integration — end-to-end realistic form
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — end-to-end realistic form', () => {
    ...
    const jobApplicationForm: FormDefinition = {
        ...
        content: [
            { id: 1, type: 'string', label: 'Full Name', validation: { required: true, minLength: 2, maxLength: 100 } },
            {
                id: 2,
                type: 'string',
                label: 'Email',
            ...
            { id: 3, type: 'number', label: 'Years of Experience', validation: { required: true, min: 0, max: 50 } },
            {
                id: 4,
                type: 'select',
                label: 'Position',
            ...
            // Conditional section: shown for senior applicants (>= 5 years)
            {
                ...
                content: [
                    { id: 8, type: 'date', label: 'Available From', validation: { required: true, minDate: '+0d' } },
    ...
    it('getAffectedIds for position field includes developer section', () => {
        ...
        // Children inside the section are not direct dependents of field 4,
        // but they ARE affected because the section controls their visibility.
        // Note: getAffectedIds returns condition-based dependents, not parent-child cascading
    })
...
describe('FormEngine — edge cases', () => {
    it('extra values not in schema are ignored', () => {
        const engine = new FormEngine(baseDef([{ id: 1, type: 'string', label: 'A', validation: { required: true } }]))
    ...
    it('validate with empty form rejects at schema level', () => {
        expect(() => new FormEngine(baseDef([]))).toThrow(FormDefinitionError)
    })
    ...
    it('string lexicographic comparison in conditions', () => {
        const engine = new FormEngine(
            baseDef([
                { id: 1, type: 'string', label: 'S' },
                {
                    id: 2,
                    type: 'string',
                    label: 'After B',
                    condition: { field: 1, op: 'gt', value: 'b' },
                },
            ]),
        )
        expect(engine.isVisible(2, doc({ '1': 'c' }))).toBe(true)
        expect(engine.isVisible(2, doc({ '1': 'a' }))).toBe(false)
    })
...
// ════════════════════════════════════════════════════════════════════════════
// 20. Visibility + Validation interplay
// ════════════════════════════════════════════════════════════════════════════

describe('FormEngine — visibility/validation interplay', () => {
